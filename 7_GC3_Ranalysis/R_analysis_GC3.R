#source('/Users/alexmckenna/Desktop/Final year/S1 Modules/FinalYearProject/1 - Project_Code/12_GC3_properties_CSV/R_analysis.R')
...
#defining col_v used in plot - error message..

col_v <- "red"
plot(c(1:29),data.t[,i],pch=20,col=col_v,main=paste("The average GC3 content at codon position in",sp2[i]),xlab="Codon",ylab=sp2[i],ylim=c(0,1))
...
df <- data.frame("Species_Accession","Species","GC3_core","The_mean_GC3_of_the_first_10_codons","Difference_between_core_and_first_10_codons_GC3","Linear_Regression_slope","Difference_First_and_second_codons_GC3","Maximum_GC3_difference_to_core","Where_max_difference","Max_Deviation_From_Core_GC3")
for (i in c(1:d.2)) {
...
#Creating CSV table
write.table(df,file="GC3_properties.csv",sep=",",row.names=FALSE,col.names=FALSE)

#Added - to confirm script ran successfully - cat function(indicates script ran without errors)
