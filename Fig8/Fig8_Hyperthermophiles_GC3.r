...
data1 <- read.csv("Hyperthermophiles_GC3.csv",header=T)
attach(data1)
...
core.gc<-unlist(data1$Core_GC) * 100
...
pdf("Fig8_Hyperthermophiles.pdf")
plot(x,m.gc, pch=20,col=col_v, xlab="Codon position", ylab="Mean GC3 (%)", ylim=c(40,55))
