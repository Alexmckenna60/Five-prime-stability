...
Temp<-unlist(data1[2])
core.gc3<-unlist(data1[5])

delta_core_f_10<-unlist(data1$Difference_between_core_and_first_10_codons_GC)

pdf("Fig14_Delta_Temp_GC3.pdf")
plot(Temp,delta_core_f_10,pch=19,cex=0.7,xlab="Optimal growth temperature (°C)",ylab=expression(Delta ~ "GC3"),col="blue")
par(pty="s")
