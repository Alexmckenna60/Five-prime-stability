...
# Output PDF
pdf("Fig_GC3_H_T_M.pdf", height=5, width=10)
par(mfrow=c(1,3))
par(pty="s")

# PLOT A - Hyperthermophiles
plot(x1, m.gc1, pch=20, col=col_v, xlab="Codon position", ylab="Mean GC3 (%)", ylim=c(0.44, 0.65),cex.lab=1.3)
lm_model1 <- lm(m.gc1 ~ x1)
abline(lm_model1, col="red")
mtext("A.",side=3,adj=0,line=1)

# PLOT B - Thermophiles
plot(x2, m.gc2, pch=20, col=col_v, xlab="Codon position", ylab="Mean GC3 (%)", ylim=c(0.45, 0.65),cex.lab=1.3)
lm_model2 <- lm(m.gc2 ~ x2)
abline(lm_model2, col="red")
mtext("B.",side=3,adj=0,line=1)

# PLOT C - Mesophiles
plot(x3, m.gc3, pch=20, col=col_v, xlab="Codon position", ylab="Mean GC3 (%)", ylim=c(0.45, 0.65),cex.lab=1.3)
