...
fout.write(f"Accession,Species,Core_GC{banner}\n")
		...
		try:		
			...
			for gn in gene_list:
				...
				for k in range(2, 30):
					cdn = codon_list[k-1]
					if cdn != "xxx":
						GC = (cdn.count("G")+cdn.count("C"))/len(cdn)
						GC_dict[k] = GC_dict[k] + GC
				...
				if len(seq) > 270:
					...
					middle_seq_g = middle_seq.count("G")
					middle_seq_c = middle_seq.count("C")
					middle_seq_gc = (middle_seq_g + middle_seq_c)/len(middle_seq)
					core_GC.append(middle_seq_gc)
					
			#Calculating average GC core
			
			m_core_gc = statistics.mean(core_GC)
			...
			outline = f"{accn},{bits[7]},{m_core_gc}"
