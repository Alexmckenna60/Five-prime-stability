data1 <- read.csv("GC_core.csv", header = TRUE)
attach(data1)

data2 <- read.csv("GC3_core.csv", header = TRUE)
attach(data2)
...
# Plot GC vs GC3
#pdf("SFig5_GC_GC3.pdf")
#plot(gc_data$Codon_Position, gc_data$Mean_GC, type = "b", col = "blue", pch = 20, ylim = c(0.4, 0.65),
#     xlab = "Codon Position", ylab = "GC Content (%)", main = "")
...
pdf("SFig5_GC_GC3.pdf")
plot(gc_data$Codon_Position, gc_data$Mean_GC, type = "b", col = "blue", pch = 20, ylim = c(40, 65), xlab = "Codon Position", ylab = "GC Content (%)", main = "")
lines(gc_data$Codon_Position, gc_data$Mean_GC3, type = "b", col = "red", pch = 20)

value <- wilcox_result$p.value

value2 <- signif(wilcox_result$p.value, digits =3)

txt<-paste("p-value =", value2)
